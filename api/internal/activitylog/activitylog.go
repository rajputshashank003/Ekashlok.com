...
// Log records that userID was active on today's IST calendar date.
//
// The call is fully idempotent — if a row already exists for (userID, today)
// the PostgreSQL ON CONFLICT clause silently ignores the duplicate, so it is
// safe to call multiple times per day without any extra guard code.
func Log(userID uint) {
	...
	result := database.DB.Exec(
		`INSERT INTO shlok_activity_logs (user_id, date) VALUES ($1, $2) ON CONFLICT (user_id, date) DO NOTHING`,
		userID, date,
	)
	if result.Error != nil {
